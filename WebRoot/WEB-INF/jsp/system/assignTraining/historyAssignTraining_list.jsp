			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="assignTraining/historyList.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<%-- <td style="padding-left:2px;"><input class="span10 date-picker" name="STARTTIME" id="STARTTIME"  value="${var.STARTTIME}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="培训开始日期"/></td>
								<td style="padding-left:2px;"><input class="span10 date-picker" name="ENDTIME" name="ENDTIME"  value="${var.ENDTIME}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="培训结束日期"/></td> --%>
								
								
								<%-- <td style="vertical-align:top;padding-left:2px;"> 
								 	<select class="chosen-select form-control" name="STATUS" id="STATUS" data-placeholder="培训状态" style="vertical-align:top;width: 79px;">
									<option value="">全部</option>
									<c:forEach items="${varList}" var="var">
									<option value="1" <c:if test="${var.STATUS == '1' }">selected</c:if> >计划了的</option>
									<option value="0" <c:if test="${var.STATUS == '0' }">selected</c:if> >完整的</option>
									</c:forEach>
									</select>
								</td> --%>
								
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="STATUZ" id="id" data-placeholder="用户状态" style="vertical-align:top;width: 68px;">
									<option value="">全部</option>
									<option value="1" <c:if test="${pd.STATUZ == '1' }">selected</c:if>>已读</option>
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.ASS_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.ASS_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.ASS_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
 	...
 	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>assignTraining/delete.do?ASS_ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>assignTraining/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>assignTraining/excel.do';
		}
	</script>
