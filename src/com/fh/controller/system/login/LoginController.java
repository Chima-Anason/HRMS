...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		/* Note: KEYDATA is an array which contains username ,password and Captcha code
		 * if KEYDATA is not null and the KEYDATA.length (KEYDATA[username, password, code]) = 3
		 */
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				//set the password you logged in with, from the session
				...
				if(Tools.isEmpty(sessionCode)){		//判断登录验证码
					...
					logger.info("加密后的密码" + passwd);     //This is the encrypted password
					
					 //The encrypted password is put into “pd”
	...
	public ModelAndView defaultPage() throws Exception{
		...
		pd.put("roleCount", Integer.parseInt(roleService.getRoleCount("").get("roleCount").toString()));	//会员数
		mv.addObject("pd",pd);
